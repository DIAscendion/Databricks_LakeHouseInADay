...
## *Author*: AAVA
## *Created on*:   
## *Description*:   Physical Data Model and DDL Scripts for Databricks Silver Layer (Shipment Domain)
## *Version*: 1 
## *Updated on*: 
_____________________________________________

# Databricks Silver Model Physical: Shipment Domain

This document defines the physical data model for the Silver layer of the Databricks Lakehouse, focusing on the SHIPMENT process and related entities. The model is designed for cleansed, conformed, and business-ready data, with robust audit and error tracking, using Delta Lake tables compatible with Databricks SQL and PySpark.

---

# Silver Layer DDL Scripts

## 1. SI_SHIPMENT_PROCESS Table

```sql
CREATE TABLE IF NOT EXISTS silver.si_shipment_process (
  shipment_process_id STRING,
  ...
  -- All Bronze columns (from bz_shipment)
...
USING DELTA
PARTITIONED BY (shipment_status, shipment_date);
```

---

## 2. SI_SHIPMENT_ITEM Table

```sql
CREATE TABLE IF NOT EXISTS silver.si_shipment_item (
...
USING DELTA
PARTITIONED BY (shipment_number);
```

---

## 3. SI_ERROR_LOG Table
...
## 5. Update DDL Script Example

```sql
-- Example: Add a new column for shipment priority
ALTER TABLE silver.si_shipment_process ADD COLUMNS (shipment_priority STRING);
```
...
- **Error/Audit Data:** 3 years from load_date

## Archiving Strategies
- Data older than retention period is moved to a separate archival storage (e.g., Azure Data Lake archival zone or S3 Glacier)
- Use Databricks OPTIMIZE and VACUUM commands for Delta tables to manage storage and performance
- Error and audit logs are periodically exported to long-term storage for compliance
...
- All Bronze columns are included in Silver tables for full lineage and traceability.
- All id fields (e.g., shipment_process_id, shipment_item_id, error_id, audit_id) are added as STRING and are expected to be generated as UUIDs or similar.
- No PK/FK/constraints are enforced in DDL (Databricks/SparkSQL limitation).
- Partitioning is chosen on business-relevant columns (shipment_status, shipment_date) for performance.
...
- Update DDL script section is provided for future schema changes.
- Data retention and archiving strategies are documented for compliance.
